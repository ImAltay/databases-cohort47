...
async function findEpisodesExercises(client) {
  ...
  const query1Result = await client
    .db('databaseWeek3')
    .collection('bob_ross_episodes')
    .findOne({ episode: 'S02E02' });

  if (query1Result) {
    console.log(`The title of episode 2 in season 2 is ${query1Result.title}`);
  }

  // Find the season and episode number of the episode called "BLACK RIVER" [Should be: S02E06]

  const query2Result = await client
    .db('databaseWeek3')
    .collection('bob_ross_episodes')
  ...
  if (query2Result) {
    console.log(
      `The season and episode number of the "BLACK RIVER" episode is ${query2Result.episode}`
    );
  }

  // Find all of the episode titles where Bob Ross painted a CLIFF [Should be: NIGHT LIGHT, EVENING SEASCAPE, SURF'S UP, CLIFFSIDE, BY THE SEA, DEEP WILDERNESS HOME, CRIMSON TIDE, GRACEFUL WATERFALL]

  let query3Result = await client
    .db('databaseWeek3')
    .collection('bob_ross_episodes')
    .find({ elements: { $in: ['CLIFF'] } });

  if (query3Result) {
    query3Result = await query3Result.toArray();
    console.log(
      `The episodes that Bob Ross painted a CLIFF are ${query3Result.map(
        (e) => ' ' + e.title
      )}`
    );
  }
  ...
  let query4Result = await client
    .db('databaseWeek3')
    .collection('bob_ross_episodes')
    .find({ elements: { $all: ['CLIFF', 'LIGHTHOUSE'] } });

  if (query4Result) {
    query4Result = await query4Result.toArray();
    console.log(
      `The episodes that Bob Ross painted a CLIFF and a LIGHTHOUSE are  ${query4Result.map(
        (e) => e.title
      )}`
    );
  }
